...
if($asal!="") {
	$ambil.=" and asal like '$asal%'";	
}
if($tiba!="") {
	$ambil.=" and Tiba like '$tiba%'";	
}
if($rute!="") {
	$ambil.=" and Rute like '%$rute%'";
}
...
<label>Cari : </label>
  ...
  <div class="form-group">
    <label class="sr-only" for="exampleInputEmail2">Rute</label>
    <input type="text" class="form-control" id="exampleInputEmail2" name="rute" placeholder="Rute" value="<?=$rute?>">
  </div>
  <div class="form-group">
            ...
            <table class="table table-hover">
              ...
              <? while($row=mysql_fetch_array($rs)) {?>
                <tr>
                  ...
                  <td><?=$row['Tiba'];?></td>
                  <td><?=$row['Rute'];?></td>
                  <td><a class="btn btn-sm btn-primary" href="?module=module/angkot/index.php&act=edit&id=<?=$row['Kode_ang'];?>" role="button">Ubah</a>
                  <a class="btn btn-sm btn-danger" onClick="return confirm('Apa anda yakin akan menghapus data ini?')" href="?module=module/angkot/remove_angkot.php&id=<?=$row['Kode_ang'];?>" role="button">Hapus</a></td>
                </tr>
              <? } ?>
              </tbody>
            </table>
